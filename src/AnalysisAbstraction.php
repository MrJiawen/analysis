...
/**
 * Class AnalysisAbstraction
 * @package CjwAnalysis\src
 */
abstract class AnalysisAbstraction
{
    // 定义两个常量
    private static $_SP_;
    private static $UCS2 = 'ucs-2be';
    ...
    /**
     * 构造函数
     * AnalysisAbstraction constructor.
    ...
    public function SetSource($source, $source_charset = 'utf-8', $target_charset = 'utf-8')
    {
        ...
        if ($source != '') {
            ...
            if (preg_match("/^utf/", $source_charset)) {
                $this->sourceString = iconv('utf-8', self::$UCS2, $source);
            } else if (preg_match("/^gb/", $source_charset)) {
                $this->sourceString = iconv('utf-8', self::$UCS2, iconv('gb18030', 'utf-8', $source));
            } else if (preg_match("/^big/", $source_charset)) {
                $this->sourceString = iconv('utf-8', self::$UCS2, iconv('big5', 'utf-8', $source));
            } else {
                $rs = false;
            }
        } else {
    ...
    public function LoadDict($maindic = '')
    {
        ...
        foreach ($ds as $d) {
            ...
            if ($estr == ':') {
                ...
                $spstr = self::$_SP_;
                $spstr = iconv(self::$UCS2, 'utf-8', $spstr);
                $ws = explode(',', $d);
                $wall = iconv('utf-8', self::$UCS2, join($spstr, $ws));
                $ws = explode(self::$_SP_, $wall);
                foreach ($ws as $estr) {
            ...
            if ($cn < 0x80) {
                if (preg_match('/' . $ansiWordMatch . '/i', chr($cn))) {
                    ...
                    if ($onstr != '') {
                        $this->simpleResult[$s]['w'] = $onstr;
                        if ($lastc == 2) {
                            if (!preg_match('/' . $notNumberMatch . '/i', iconv(self::$UCS2, 'utf-8', $onstr))) $lastc = 4;
                        }
                        $this->simpleResult[$s]['t'] = $lastc;
    ...
    private function _out_string_encoding(&$str)
    {
        ...
        if ($rsc == 1) {
            $rsstr = iconv(self::$UCS2, 'utf-8', $str);
        } else if ($rsc == 2) {
            $rsstr = iconv('utf-8', 'gb18030', iconv(self::$UCS2, 'utf-8', $str));
        } else {
            $rsstr = iconv('utf-8', 'big5', iconv(self::$UCS2, 'utf-8', $str));
        }
        return $rsstr;
    }
